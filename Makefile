...
kernel.bin: kernel/kernel.c kernel/disk.c kernel/disk.h kernel/pmm.c kernel/paging.c kernel/paging.asm kernel/interrupt.asm kernel/idt.c kernel/syscall.c kernel/graphics.c kernel/pci.c kernel/ahci.c kernel/ahci.h kernel/scheduler.c kernel/shell.c kernel/input.c kernel/dashboard.c kernel/storage_manager.c kernel/klog.c kernel/vfs.c kernel/net.c kernel/net.h kernel/profiling.c kernel/profiling.h kernel/model_manager.c kernel/model_manager.h kernel/multiboot.asm kernel/usermode.c kernel/usermode.asm kernel/wm.c kernel/wm.h kernel/font.h linker.ld kernel/launcher.c kernel/launcher.h
	...
	$(CC) $(CFLAGS) -c kernel/kernel.c -o kernel.o
	$(LD) -o $@ -T linker.ld multiboot.o kernel.o disk.o pmm.o paging.o paging_asm.o interrupt.o switch.o usermode_asm.o task.o scheduler.o idt.o syscall.o graphics.o pci.o ahci.o shell.o input.o dashboard.o storage_manager.o klog.o vfs.o net.o profiling.o model_manager.o usermode.o launcher.o wm.o --oformat binary

# Clean build artifacts
clean:
	rm -f NeuroSpark.bin kernel.bin kernel.o disk.o pmm.o paging.o paging_asm.o interrupt.o switch.o usermode_asm.o task.o scheduler.o idt.o syscall.o graphics.o pci.o ahci.o shell.o input.o dashboard.o storage_manager.o klog.o vfs.o net.o profiling.o model_manager.o usermode.o wm.o launcher.o boot/boot.bin
